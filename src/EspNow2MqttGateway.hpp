...
void EspNow2Mqtt_mqttCallback(char *topic, uint8_t *payload, unsigned int length)
{
    ...
    if (instance)
    {
        if (payload)
        {
            instance->topics[String(topic)] = String(payload, length);
        }
        else
        {
...
EspNow2MqttGateway::EspNow2MqttGateway(byte key[16], Client &cnx, const char *mqttServer, const int mtttPort, const char *mqttID, const char *mqttUser, const char *mqttPassword) : crmsg(key, sizeof(byte) * 16), eNowUtil(), mqttClient(mqttServer, mtttPort, EspNow2Mqtt_mqttCallback, cnx)
{
...
void EspNow2MqttGateway::subscribeHandler(const uint8_t *mac_addr, char *clientId, request_Subscribe &subs, response_OpResponse &rsp)
{
    ...
    if (subscriptions.find(queue) != subscriptions.end())
    { // subscribed
